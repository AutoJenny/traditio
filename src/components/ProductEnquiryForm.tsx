...
export default function ProductEnquiryForm({ productSlug }: { productSlug: string }) {
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      const res = await fetch("/api/enquiry", {
        ...
        body: JSON.stringify({ ...form, productSlug, pageUrl: window.location.pathname }),
      });
  ...
  return (
    <div className="mt-8">
      {submitted ? (
        <div className="bg-green-100 text-green-800 p-4 rounded mb-4">Thank you for your enquiry! We'll be in touch soon.</div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4 bg-sand-50 p-6 rounded shadow">
          ...
          <button type="submit" disabled={loading} className="bg-brass text-espresso font-bold rounded px-6 py-2 border-2 border-brass shadow hover:bg-espresso hover:text-ivory transition">{loading ? "Sending..." : "Send Enquiry"}</button>
        </form>
      )}
    </div>
  );
